...
export default function AlertFailedModal() {
  const { failedModal, closeFailedModal } = alertModalStore(
    (state) => state,
    shallow,
  );

  const { isOpen, title, message, closeNameBtn } = failedModal ?? {};
              ...
              >
                <div className=" p-5">
                  <Dialog.Title className="flex flex-row items-center gap-1">
                    <Io5icons.IoCheckmarkCircle className="text-3xl text-stateRed" />
                ...
                <div className="flex justify-end gap-5 rounded-b-xl bg-lightRed p-3">
                  <button
                    ...
                    className="btn btn-error rounded-xl px-10 font-normal capitalize text-white"
                  >
                    {closeNameBtn}
                  </button>
                </div>
